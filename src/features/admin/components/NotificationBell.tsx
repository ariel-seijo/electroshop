...
export default function NotificationBell({ lowStock, recentOrders, pendingCount }: NotificationBellProps) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute top-[calc(100%+8px)] right-0 w-[260px] bg-surface-14 border border-white/10 rounded-lg shadow-[0_12px_32px_rgba(0,0,0,0.6)] z-50 overflow-hidden animate-[popIn_0.15s_ease-out] max-md:fixed max-md:top-[var(--admin-header-height,56px)] max-md:left-0 max-md:right-0 max-md:w-auto max-md:max-w-none max-md:rounded-none max-md:border-x-0 max-md:border-t-0 max-md:shadow-[0_16px_40px_rgba(0,0,0,0.8)] max-md:animate-[slideDown_0.22s_cubic-bezier(0.4,0,0.2,1)] max-md:max-h-[calc(100vh-var(--admin-header-height,56px))] max-md:overflow-y-auto" role="dialog" aria-label="Notificaciones">
          <div className="flex items-center justify-between px-[14px] py-3 text-accent max-md:px-4 max-md:py-[14px] max-md:text-xs border-b border-white/5">
          ...
          <div className="max-h-[360px] overflow-y-auto">
              ...
              <div>
                ...
                {orderItems.map((order) => (
                  <div key={`order-${order.id}`} className={itemBase}>
                    ...
                    <button type="button" className="flex items-center justify-center min-w-7 min-h-7 border-none bg-transparent text-[rgb(100,100,100)] cursor-pointer rounded shrink-0 mt-0.5 transition-colors duration-[150ms] hover:text-danger hover:bg-danger/10 max-md:min-w-9 max-md:min-h-9" onClick={() => handleDismiss(`order-${order.id}`)} aria-label={`Descartar alerta de ${order.orderNumber}`}>×</button>
                  </div>
                ))}
              </div>
            )}
            ...
            {lowStockItems.length === 0 && orderItems.length === 0 && (
              <div className="px-[14px] py-5 text-[0.78rem] text-text-placeholder text-center max-md:px-4 max-md:py-6 max-md:text-[0.82rem]">
